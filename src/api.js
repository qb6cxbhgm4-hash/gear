const API = "https://saas-production-c2dc.up.railway.app";
...
const api = {
  ...
  async products() {
    ...
    const list = r.products || (Array.isArray(r) ? r : []);
    return list.map(mp);
  },
  async product(id) {
  ...
  async alerts() {
    ...
    const list = r.alerts || (Array.isArray(r) ? r : []);
    return list.map((a) => ({
  ...
  async disconnectStore(id) {
    return call(`/store/${id}`, { method: "DELETE" });
  },
  async oauthCallback(sessionToken, shop) {
    return call("/store/oauth/callback", {
      method: "POST",
